...
def fun3(json_path, wsi_path, output_path, xml_path=None):
    ...
    extension = os.path.basename(wsi_path).split('.')[-1]
    sample_name = os.path.basename(wsi_path).split(f".{extension}")[0]
    with open(json_path) as fp:
        print(f"{'Loading json':*^30s}")
        nucleusInfo = json.load(fp)

    globalgraph, morphFeats, GLCMFeats = constructGraphFromDict(wsi_path, nucleusInfo, distanceThreshold, k, level)
    ...
    print(type(morphFeats))
    with open(os.path.join(output_path, sample_name + "MorphFeatures.pkl"), 'wb') as file:
        pickle.dump(morphFeats, file)

    with open(os.path.join(output_path, sample_name + "GLCMFeatures.pkl"), 'wb') as file:
        pickle.dump(GLCMFeats, file)