...
def getCellImg(slidePtr, bbox, pad=2, level=0, gray=True):
    bbox = np.array(bbox)
    bbox[0] = bbox[0] - pad
    bbox[1] = bbox[1] + pad
...
def SingleGLCMFeatures(args):
    ...
    for contour, bbox in zip(contours, bboxes):
        ...
        cellImgColor = getCellImg(slidePtr, bbox, pad, level, gray=False)
        ...
        ## Add color features
        mean_color_R, mean_color_G, mean_color_B, mean_transparency = np.mean(cellImgColor, axis=(0, 1))
        std_color_R, std_color_G, std_color_B, std_transparency = np.std(cellImgColor, axis=(0, 1))
        skew_color_R, skew_color_G, skew_color_B = skew(cellImgColor[:, :, :3], axis=(0, 1))
        kurtosis_color_R, kurtosis_color_G, kurtosis_color_B = kurtosis(cellImgColor[:, : ,:3], axis=(0, 1))
        entropy_R = shannon_entropy(cellImgColor[:, :, 0])
        entropy_G = shannon_entropy(cellImgColor[:, :, 1])
        entropy_B = shannon_entropy(cellImgColor[:, :, 2])
        
        featuresDict['MeanColor_R'] += [mean_color_R]
        featuresDict['MeanColor_G'] += [mean_color_G]
        featuresDict['MeanColor_B'] += [mean_color_B]
        featuresDict['MeanTransparency'] += [mean_transparency]
        featuresDict['StdColor_R'] += [std_color_R]
        featuresDict['StdColor_G'] += [std_color_G]
        featuresDict['StdColor_B'] += [std_color_B]
        featuresDict['StdTransparency'] += [std_transparency]
        featuresDict['SkewColor_R'] += [skew_color_R]
        featuresDict['SkewColor_G'] += [skew_color_G]
        featuresDict['SkewColor_B'] += [skew_color_B]
        featuresDict['KurtosisColor_R'] += [kurtosis_color_R]
        featuresDict['KurtosisColor_G'] += [kurtosis_color_G]
        featuresDict['KurtosisColor_B'] += [kurtosis_color_B]
        featuresDict['Entropy_Color_R'] += [entropy_R]
        featuresDict['Entropy_Color_G'] += [entropy_G]
        featuresDict['Entropy_Color_B'] += [entropy_B]
...
def constructGraphFromDict(
    ...
    return globalGraph, morphFeats, GLCMFeats
    # t3 = time.time()
    ...
    # return globalGraph, edge_info, morphFeats, GLCMFeats
